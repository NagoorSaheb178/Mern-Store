...
router.get("/category/:category", async (req,res)=>{
  const db = await getDb();
  const list = await db.collection("products").find({ category:req.params.category }).sort({ createdAt:-1 }).toArray();
  res.json(list.map(p=>({ ...p, _id:p._id.toString() })));
});
...
router.post("/", authMiddleware, async (req,res)=>{
  ...
  const db = await getDb();
  const doc = { title, price:Number(price), description:description||"", category:category||"general",
    imageUrl:imageUrl||"", imageBase64:imageBase64||"", createdAt:new Date(), updatedAt:new Date() };
...
router.delete("/:id", authMiddleware, async (req,res)=>{
  const db = await getDb();
  const r = await db.collection("products").deleteOne({ _id:new ObjectId(req.params.id) });
...
router.put("/:id", authMiddleware, async (req,res)=>{
  ...
  const u = await db.collection("products").findOneAndUpdate(
    ...
    { $set:{ title, price:Number(price), description:description||"", category:category||"general",
      imageUrl:imageUrl||"", imageBase64:imageBase64||"", updatedAt:new Date() } },
    { returnDocument:"after" }
  );
  if(!u.value) return res.status(404).json({ message:"Product not found" });
